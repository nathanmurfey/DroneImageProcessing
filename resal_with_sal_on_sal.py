...
def get_boxes(contours, mask, buff):
    boxes = []
    for cnt in contours:
...
def draw_detections(frame, contours):
    for cnt in contours:
        cv2.drawContours(frame, [cnt], 0, (0, 255, 100), 1)
...
while (cam.isOpened()):
    ...
    if ((counter % 1) == 0):
        ...
        boxes = get_boxes(contour_filter_one_out, mask, 0)

        #hulls = convert_to_hull(contour_filter_one_out)
        cv2.drawContours(mask, boxes,-1, (255, 255, 255), -1)
        ...
        # background
        fill_in_frame = cv2.bitwise_and(sal_conv,sal_conv,mask = mask)

        # foreground
        foreground_frame = np.ones(frame.shape, np.uint8)
        ...
        # sal again
        fill_in_frame = cv2.cvtColor(fill_in_frame, cv2.COLOR_GRAY2BGR)
