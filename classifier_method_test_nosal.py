...
for i in range(0,frame_count):
    ...
    for (x, y, w, h) in rects:
        ...
        top_down = 50
        across = 100
        display[top_down:top_down+193,1000+across:1096+across,:] = object
        faces = haar_classifier.detect_faces(object)
        print faces, "--<"

        cv2.putText(display, "HOG :" + str(round(weights[0][0], 3)), (1000+5, top_down+193+60), font, 1, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    for (x, y, w, h) in bodies:
        ...
        if area > 15000:
            cv2.rectangle(test_image, (x, y), (x + w, y + h), (0, 255, 0), 2)
            cv2.putText(display, "HRB :1.0/" + str(round(len(bodies), 1)), (1000 + 5, top_down + 193 + 60+30), font, 1,(255, 255, 255), 1, cv2.LINE_AA)

    for (x, y, w, h) in upper:
        cv2.rectangle(test_image, (x, y), (x + w, y + h), (255, 0, 0), 2)

    cv2.putText(display, "HRU :1.0/" + str(round(len(upper), 1)), (1000 + 5, top_down + 193 + 60 + 30 + 30), font, 1, (255, 255, 255), 1, cv2.LINE_AA)

    for (x, y, w, h) in lower:
        cv2.rectangle(test_image, (x, y), (x + w, y + h), (0, 0, 255), 2)
    cv2.putText(display, "HRL :1.0/" + str(round(len(lower), 1)), (1000 + 5, top_down + 193 + 60 + 30 + 30 + 30), font, 1,(255, 255, 255), 1, cv2.LINE_AA)
